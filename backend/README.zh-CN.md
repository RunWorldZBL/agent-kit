...
| Harness 角色            | 组件                                                      | 作用                             |
| ----------------------- | --------------------------------------------------------- | -------------------------------- |
| **Guide** (前馈引导)    | `AGENTS.md`                                               | 上下文工程：规则、约定、响应格式 |
| **Guide** (前馈引导)    | Skills (`$crud`, `$db-schema`, `$bullmq`, `$drizzle-v1`)  | 结构化模板，预引导代码生成       |
| **Guide** (前馈引导)    | OpenSpec (`$openspec-propose` → `$openspec-apply-change`) | 规划层：规范成为首要产物         |
...
越投入 Harness 的专精化（添加 Skills、完善 AGENTS.md、扩展测试），AI Agent 的输出越可靠。这适用于任何 AI 编程 Agent。
...
- **OpenSpec 工作流**: [OpenSpec](https://github.com/Fission-AI/OpenSpec) AI 原生变更管理（`$openspec-propose` → `$openspec-apply-change` → `$openspec-archive-change`），将规范转化为首要开发产物
- **AGENTS.md 上下文工程**: 完整的项目规则、约定、响应格式和工作流编排，实现 AI Agent 对齐
...
**验收标准**：所有测试通过 + 符合 AGENTS.md 规范 + 无明显性能问题
...
```text
$openspec-propose "add-user-export"    # 创建变更 + 生成所有规划制品
$openspec-apply-change                        # 按任务列表执行实现
$openspec-archive-change                      # 归档已完成的变更
$openspec-explore                      # 在正式变更前探索想法
```

生成的制品位于 `openspec/changes/<name>/`（已 gitignore）。在 Codex 中请从根目录 `.codex/skills/` 以 skill 方式调用这些流程。

### 不局限于某一个 Agent 客户端

虽然本模板最初使用 Codex Skills，但 SDD + TDD 方法论和 Harness 组件适用于 Codex 或自建的驾驭系统。Schema 流水线、测试基础设施和架构约束对任何开发方式都有益 — 无论是人工还是 AI 辅助。
...
将所有 JS 依赖打包为单文件，原生二进制提取到 `dist/native/`，无需 `node_modules` 即可部署。

```ts
// vite.config.ts
buildPluginNodejs({
  bundleDeps: true, // 将所有 JS 依赖打包为单文件
  nativeDeps: ["@node-rs/argon2", "excelize-wasm"], // 提取 .node/.wasm 二进制到 dist/native/
  targetPlatform: "linux-x64", // 交叉编译：在 macOS 上安装 linux 二进制
});
```

| 选项             | 说明                                                                                                |
| ---------------- | --------------------------------------------------------------------------------------------------- |
| `bundleDeps`     | `true` 将所有 JS 打包到 `dist/index.js`；`false`（默认）保持 `node_modules` external，适用于 Docker |
| `nativeDeps`     | 含 `.node`/`.wasm` 二进制的 npm 包 — JS 正常打包，二进制提取到 `dist/native/`                       |
| `targetPlatform` | 交叉编译目标平台：`linux-x64` \| `linux-arm64` \| `darwin-arm64` \| `darwin-x64` \| `win32-x64`     |

产物结构：

```
dist/
  index.js                        # 单文件 bundle（所有 JS 依赖已打包）
  native/
    argon2.linux-x64-gnu.node     # 平台特定的原生插件
    argon2.linux-x64-musl.node    # Alpine Linux 变体
    excelize.wasm.gz              # WASM 二进制
```
...
### Native Assets（自有原生文件）

`nativeAssets` 用于将项目中自定义的 `.wasm`/`.node` 等二进制文件复制到 `dist/native/`。**不依赖 `bundleDeps`**，Docker 模式和独立部署模式均可使用。

```ts
// vite.config.ts — 仅 nativeAssets（Docker 模式也可用）
buildPluginNodejs({
  nativeAssets: ["src/workers/xxxxxx.wasm"],
});
```

运行时加载示例（优先 `native/` 产物目录，兜底源码路径）：
...
function getWasmPath(): string {
  // 优先 dist/native/（nativeAssets 构建产物）
...
本模板将 AI 编程 Agent 视为一等开发界面。Harness 围绕 Agent 在改代码前可以读取的显式文件构建。

### Harness 组件

| 组件                  | 路径 / 调用方式                    | 用途                                         |
| --------------------- | ---------------------------------- | -------------------------------------------- |
| **AGENTS.md**         | `./AGENTS.md`                      | 系统提示词：规则、约定、响应格式、工作流编排 |
...
| **OpenSpec: Propose** | `$openspec-propose`                | 创建变更及所有规划制品                       |
| **OpenSpec: Apply**   | `$openspec-apply-change`           | 按变更执行任务                               |
| **OpenSpec: Explore** | `$openspec-explore`                | 设计探索的思考伙伴                           |
| **OpenSpec: Archive** | `$openspec-archive-change`         | 归档已完成的变更                             |
...
`AGENTS.md` 文件是最关键的 Harness 文档。它包含：
...
AGENTS.md 中的每条规则都是一个前馈约束，防止 AI Agent 偏离正确模式。
