...
| ----------------------- | --------------------------------------------------------- | ---------------------------------------------------------- |
| **Guide** (Feedforward) | `AGENTS.md`                                               | Context engineering: rules, conventions, response patterns |
| **Guide** (Feedforward) | Skills (`$crud`, `$db-schema`, `$bullmq`, `$drizzle-v1`)  | Structured templates that pre-steer code generation        |
| **Guide** (Feedforward) | OpenSpec (`$openspec-propose` → `$openspec-apply-change`) | Planning layer: specs become the primary artifact          |
...
The more you invest in specializing the harness (adding skills, refining AGENTS.md, expanding tests), the more reliably the AI agent produces correct code. This applies to any AI coding agent.
...
- **OpenSpec Workflow**: [OpenSpec](https://github.com/Fission-AI/OpenSpec) AI-native change management (`$openspec-propose` → `$openspec-apply-change` → `$openspec-archive-change`), turning specifications into the primary development artifact
- **AGENTS.md Context Engineering**: Comprehensive project rules, conventions, response patterns, and workflow orchestration for AI agent alignment
...
| --------- | ---------------------------------------------- | -------------------------------------------------------------- | -------------- |
| Spec      | `$openspec-propose`                            | `openspec/changes/<name>/` (proposal, design, tasks)           | Guide          |
...
**Acceptance Criteria**: All tests pass + complies with AGENTS.md specs + no obvious performance issues
...
```text
$openspec-propose "add-user-export"    # Create change + generate all planning artifacts
...
Generated artifacts live in `openspec/changes/<name>/` (gitignored). In Codex, invoke these workflows as root skills from `.codex/skills/`.

### Not Limited to One Agent Client

While this template started with Codex skills, the SDD + TDD methodology and harness components work with Codex or a custom-built harness system. The schema pipeline, test infrastructure, and architectural constraints benefit any development approach — human or AI-assisted.
...
## Harness System: Agent Integration

This template treats the AI coding agent as a first-class development interface. The harness is built around explicit files that the agent can read before making changes.

### Harness Components

| Component             | Path / Invocation                  | Purpose                                                                      |
| --------------------- | ---------------------------------- | ---------------------------------------------------------------------------- |
| **AGENTS.md**         | `./AGENTS.md`                      | System prompt: rules, conventions, response patterns, workflow orchestration |
...
| **OpenSpec: Propose** | `$openspec-propose`                | Create change with all planning artifacts                                    |
| **OpenSpec: Apply**   | `$openspec-apply-change`           | Implement tasks from a change                                                |
| **OpenSpec: Explore** | `$openspec-explore`                | Thinking partner for design exploration                                      |
| **OpenSpec: Archive** | `$openspec-archive-change`         | Archive completed changes                                                    |
...
### AGENTS.md as Context Engineering

The `AGENTS.md` file is the most critical harness document. It contains:
...
Every rule in AGENTS.md is a feedforward constraint that prevents the AI agent from deviating into incorrect patterns.
